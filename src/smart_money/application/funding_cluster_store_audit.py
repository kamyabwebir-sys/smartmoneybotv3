...
_SCHEMA_VERSION = "funding_cluster_store_audit.v1"


@dataclass(frozen=True, slots=True)
class FundingClusterStoreAuditReceipt:
    cluster_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version != _SCHEMA_VERSION:
            raise ValueError("unsupported funding cluster store audit schema_version")
        if self.audit_id != deterministic_id("funding_cluster_store_audit", self.identity_payload()):
    ...
    def identity_payload(self) -> dict[str, Any]:
        return {
            ...
            "store_replay_id": self.store_replay_id,
        }
...
def build_funding_cluster_store_audit_receipt(
    *,
    cluster_id: str,
    ledger_receipt: FundingClusterLedgerReceipt,
    replay_receipt: FundingClusterReplayReceipt,
    store_replay_receipt: FundingClusterStoreReplayReceipt,
    ...
    if not isinstance(store_replay_receipt, FundingClusterStoreReplayReceipt):
        raise TypeError("store_replay_receipt must be FundingClusterStoreReplayReceipt")
    ...
    if store_replay_receipt.model_id.strip() == "":
        raise ValueError("store replay model_id must be non-empty")
    identity = {
        "cluster_id": normalized,
        "ledger_receipt_id": ledger_receipt.receipt_id,
        "replay_id": replay_receipt.replay_id,
        "replay_matches": replay_receipt.matches and store_replay_receipt.matches,
        "schema_version": _SCHEMA_VERSION,
        "store_replay_id": store_replay_receipt.replay_id,
    }
    return FundingClusterStoreAuditReceipt(
        **identity,
...
__all__ = [
    "FundingClusterStoreAuditReceipt",
    "build_funding_cluster_store_audit_receipt",
]
