...
_SCHEMA_VERSION = "wallet_intelligence_ledger.v1"


@dataclass(frozen=True, slots=True)
class WalletIntelligenceLedgerReceipt:
    receipt_id: str
    observation_id: str
    evidence_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version != _SCHEMA_VERSION:
            raise ValueError("unsupported wallet intelligence ledger schema_version")
        if self.receipt_id != deterministic_id(
            "wallet_intelligence_ledger", self.identity_payload()
        ):
...
def ingest_wallet_intelligence_observation(
    observation: WalletIntelligenceObservation, ledger: EvidenceLedger
) -> WalletIntelligenceLedgerReceipt:
    ...
    payload = EvidencePayload(
        ...
        evidence_type="wallet_intelligence_observation",
        timestamp=observation.observed_to_slot,
        data={"wallet_intelligence": observation.canonical_dict()},
        metadata={
            "authority": "NONE",
    ...
    if ledger.append(payload) != evidence_id or ledger.get(evidence_id) != payload:
        raise ValueError("Ledger retained wallet intelligence payload mismatch")
    ...
    return WalletIntelligenceLedgerReceipt(
        receipt_id=deterministic_id("wallet_intelligence_ledger", identity),
        observation_id=observation.observation_id,
        evidence_id=evidence_id,
        ledger_entry_count=ledger.entry_count,
        already_present=already_present,
...
__all__ = ["WalletIntelligenceLedgerReceipt", "ingest_wallet_intelligence_observation"]
