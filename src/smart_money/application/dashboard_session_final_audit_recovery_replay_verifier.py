...
class DashboardSessionFinalAuditRecoveryReplayReceipt:
    expected_receipt_id: str
    actual_receipt_id: str
    matches: bool
    mismatches: tuple[str, ...] = ()
    schema_version: str = "dashboard_session_final_audit_recovery_replay.v1"
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.schema_version
            != "dashboard_session_final_audit_recovery_replay.v1"
        ):
            raise ValueError(
                "unsupported session final audit recovery replay schema_version"
            )
    ...
    def verification_id(self) -> str:
        return deterministic_id(
            "dashboard_session_final_audit_recovery_replay",
            self.canonical_dict(),
        )


@dataclass(frozen=True, slots=True)
class DashboardSessionFinalAuditRecoveryReplayVerifier:
    gate: DashboardSessionFinalAuditRecoveryGate = (
        DashboardSessionFinalAuditRecoveryGate()
    )

    def verify(
        self,
        expected: DashboardSessionFinalAuditRecoveryReceipt,
        ...
        if not isinstance(
            ...
            raise TypeError(
                "expected must be a DashboardSessionFinalAuditRecoveryReceipt"
            )
        actual = self.gate.evaluate(chain, replay, persisted_replay)
        mismatches = tuple(
            field
            for field in ("decision", "reason_code", "chain_id", "replay_verification_id")
            if getattr(expected, field) != getattr(actual, field)
        )
        return DashboardSessionFinalAuditRecoveryReplayReceipt(
