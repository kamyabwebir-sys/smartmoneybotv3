...
class DashboardSessionFinalRecoveryGateAuditChainReplayReceipt:
    ...
    schema_version: str = (
        "dashboard_session_final_recovery_gate_audit_chain_replay.v1"
    )
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "unsupported final recovery gate audit chain replay schema_version"
            )
    ...
    def verification_id(self) -> str:
        return deterministic_id(
            "dashboard_session_final_recovery_gate_audit_chain_replay",
            self.canonical_dict(),
        )


@dataclass(frozen=True, slots=True)
class DashboardSessionFinalRecoveryGateAuditChainReplayVerifier:
    def verify(
        self,
        expected: DashboardSessionFinalRecoveryGateAuditChain,
        ...
        if not isinstance(
            ...
            raise TypeError(
                "expected must be a DashboardSessionFinalRecoveryGateAuditChain"
            )
        if not isinstance(
            actual, DashboardSessionFinalRecoveryGateAuditChain
        ):
            raise TypeError(
                "actual must be a DashboardSessionFinalRecoveryGateAuditChain"
            )
        mismatches = tuple(
            field
        ...
        return DashboardSessionFinalRecoveryGateAuditChainReplayReceipt(
