...
class DashboardSessionFinalRecoveryAuditChainReplayReceipt:
    ...
    schema_version: str = (
        "dashboard_session_final_recovery_audit_chain_replay.v1"
    )
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "unsupported final recovery audit chain replay schema_version"
            )
    ...
    def verification_id(self) -> str:
        return deterministic_id(
            "dashboard_session_final_recovery_audit_chain_replay",
            self.canonical_dict(),
        )


@dataclass(frozen=True, slots=True)
class DashboardSessionFinalRecoveryAuditChainReplayVerifier:
    def verify(
        self,
        expected: DashboardSessionFinalRecoveryAuditChain,
        actual: DashboardSessionFinalRecoveryAuditChain,
    ) -> DashboardSessionFinalRecoveryAuditChainReplayReceipt:
        if not isinstance(expected, DashboardSessionFinalRecoveryAuditChain):
            raise TypeError(
                "expected must be a DashboardSessionFinalRecoveryAuditChain"
            )
        if not isinstance(actual, DashboardSessionFinalRecoveryAuditChain):
            raise TypeError(
                "actual must be a DashboardSessionFinalRecoveryAuditChain"
            )
        mismatches = tuple(
            field
        ...
        return DashboardSessionFinalRecoveryAuditChainReplayReceipt(
