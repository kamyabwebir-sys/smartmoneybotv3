...
_SCHEMA_VERSION = "dashboard_session_final_audit_chain_replay_store.v1"


class JsonDashboardSessionFinalAuditChainReplayStore:
    """Atomic, append-only persistence for final chain replay receipts."""
    ...
    def __init__(self, file_path: str | os.PathLike[str]) -> None:
        ...
        self._receipts: dict[
            str, DashboardSessionFinalAuditChainReplayReceipt
        ] = {}
        self._load()

    def append(
        self,
        receipt: DashboardSessionFinalAuditChainReplayReceipt,
    ) -> str:
        if not isinstance(
            receipt, DashboardSessionFinalAuditChainReplayReceipt
        ):
            raise TypeError(
                "receipt must be a DashboardSessionFinalAuditChainReplayReceipt"
            )
        existing = self._receipts.get(receipt.verification_id)
        if existing is not None:
            if existing != receipt:
                raise RuntimeError(
                    "session final audit chain replay identity collision"
                )
    ...
    def get(
        self,
        verification_id: str,
    ) -> DashboardSessionFinalAuditChainReplayReceipt | None:
        return self._receipts.get(verification_id)

    def iter_receipts(
        self,
    ) -> Iterator[DashboardSessionFinalAuditChainReplayReceipt]:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "session final audit chain replay store is not valid JSON"
            ) from exc
        if not isinstance(document, dict) or set(document) != {
            ...
            raise ValueError(
                "session final audit chain replay store keys do not match schema"
            )
        if document["schema_version"] != _SCHEMA_VERSION:
            raise ValueError(
                "unsupported session final audit chain replay store schema_version"
            )
        ...
        if not isinstance(records, list) or _digest(records) != document[
            ...
            raise ValueError(
                "session final audit chain replay store content hash mismatch"
            )
        if canonical_json(document) != raw:
            raise ValueError(
                "session final audit chain replay store is not canonical JSON"
            )
        loaded: dict[str, DashboardSessionFinalAuditChainReplayReceipt] = {}
        for record in records:
            try:
                ...
                raise ValueError(
                    "invalid session final audit chain replay receipt"
                ) from exc
            if receipt.verification_id in loaded:
                raise ValueError(
                    "duplicate session final audit chain replay verification ID"
                )
    ...
    def _persist(
        self,
        receipts: tuple[DashboardSessionFinalAuditChainReplayReceipt, ...],
    ) -> None:
...
__all__ = ["JsonDashboardSessionFinalAuditChainReplayStore"]
