...
def _parse(data: dict[str, Any]) -> DashboardSessionFinalRecoveryChainReceipt:
    return DashboardSessionFinalRecoveryChainReceipt(**data)


_MANIFEST = StoreManifest(
    schema_version=_SCHEMA_VERSION,
    collection_key="receipts",
    id_field="receipt_id",  # @property on the receipt
    parse=_parse,
...
class JsonDashboardSessionFinalRecoveryChainReceiptStore:
    ...
    def append(self, receipt: DashboardSessionFinalRecoveryChainReceipt) -> str:
        return self._inner.append(receipt)

    def get(self, receipt_id: str) -> DashboardSessionFinalRecoveryChainReceipt | None:
