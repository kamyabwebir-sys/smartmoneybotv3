...
_SCHEMA_VERSION = (
    "dashboard_session_final_recovery_gate_audit_session_integration_mapping_chain_level_persistence_replay_store_chain_binding_persistence_store.v1"
)
_Receipt = (
    DashboardSessionFinalRecoveryGateAuditSessionIntegrationMappingChainLevelPersistenceReplayStoreChainBindingPersistenceAuditReceipt
)
...
class JsonDashboardSessionFinalRecoveryGateAuditSessionIntegrationMappingChainLevelPersistenceReplayStoreChainBindingPersistenceStore:  # noqa: E501
    """Atomic persistence for M71.10 chain-binding persistence audit receipts."""
    ...
    def append(self, receipt: _Receipt) -> str:
        if not isinstance(receipt, _Receipt):
            raise TypeError("receipt must be a chain-binding persistence audit receipt")
        receipt_id = receipt.audit_id
        existing = self._receipts.get(receipt_id)
        if existing is not None:
            if existing != receipt:
                raise RuntimeError("chain-binding persistence receipt identity collision")
            return receipt_id
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ValueError("chain-binding persistence store is not valid JSON") from exc
        if not isinstance(document, dict) or set(document) != {
            "content_hash",
            "receipts",
            "schema_version",
        }:
            raise ValueError("chain-binding persistence store keys do not match schema")
        if document["schema_version"] != _SCHEMA_VERSION:
            raise ValueError("unsupported chain-binding persistence store schema_version")
        records = document["receipts"]
        if not isinstance(records, list) or _digest(records) != document["content_hash"]:
            raise ValueError("chain-binding persistence store content hash mismatch")
        if canonical_json(document) != raw:
            raise ValueError("chain-binding persistence store is not canonical JSON")
        loaded: dict[str, _Receipt] = {}
        for record in records:
            try:
                receipt = _Receipt(
                    expected_audit_id=record["expected_audit_id"],
                    persisted_audit_id=record["persisted_audit_id"],
                    matches=record["matches"],
                    mismatches=tuple(record["mismatches"]),
                    schema_version=record["schema_version"],
                )
            except (KeyError, TypeError, ValueError) as exc:
                raise ValueError("invalid chain-binding persistence receipt") from exc
            if receipt.audit_id in loaded:
                raise ValueError("duplicate chain-binding persistence receipt ID")
