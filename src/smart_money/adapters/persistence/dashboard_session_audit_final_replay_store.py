...
_SCHEMA_VERSION = "dashboard_session_audit_final_replay_store.v1"


class JsonDashboardSessionAuditFinalReplayStore:
    """Atomic, append-only persistence for final audit replay receipts."""
    ...
    def __init__(self, file_path: str | os.PathLike[str]) -> None:
        ...
        self._receipts: dict[
            str, DashboardSessionAuditFinalReplayReceipt
        ] = {}
        self._load()

    def append(self, receipt: DashboardSessionAuditFinalReplayReceipt) -> str:
        if not isinstance(receipt, DashboardSessionAuditFinalReplayReceipt):
            raise TypeError(
                "receipt must be a DashboardSessionAuditFinalReplayReceipt"
            )
        existing = self._receipts.get(receipt.verification_id)
        if existing is not None:
            if existing != receipt:
                raise RuntimeError(
                    "session audit final replay identity collision"
                )
    ...
    def get(
        self,
        verification_id: str,
    ) -> DashboardSessionAuditFinalReplayReceipt | None:
        return self._receipts.get(verification_id)

    def iter_receipts(
        self,
    ) -> Iterator[DashboardSessionAuditFinalReplayReceipt]:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "session audit final replay store is not valid JSON"
            ) from exc
        if not isinstance(document, dict) or set(document) != {
            ...
            raise ValueError(
                "session audit final replay store keys do not match schema"
            )
        if document["schema_version"] != _SCHEMA_VERSION:
            raise ValueError(
                "unsupported session audit final replay store schema_version"
            )
        ...
        if not isinstance(records, list) or _digest(records) != document[
            ...
            raise ValueError(
                "session audit final replay store content hash mismatch"
            )
        if canonical_json(document) != raw:
            raise ValueError(
                "session audit final replay store is not canonical JSON"
            )
        loaded: dict[str, DashboardSessionAuditFinalReplayReceipt] = {}
        for record in records:
            try:
                receipt = DashboardSessionAuditFinalReplayReceipt(
                ...
                raise ValueError(
                    "invalid session audit final replay receipt"
                ) from exc
            if receipt.verification_id in loaded:
                raise ValueError(
                    "duplicate session audit final replay verification ID"
                )
    ...
    def _persist(
        self,
        receipts: tuple[DashboardSessionAuditFinalReplayReceipt, ...],
    ) -> None:
...
__all__ = ["JsonDashboardSessionAuditFinalReplayStore"]
