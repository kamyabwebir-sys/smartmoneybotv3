...
_SCHEMA_VERSION = "wallet_relationship_intelligence_audit_store.v1"


class JsonWalletRelationshipIntelligenceAuditStore:
    def __init__(self, file_path: str | os.PathLike[str]) -> None:
        self._file_path = Path(file_path)
        self._receipts: dict[str, WalletRelationshipIntelligenceAuditReceipt] = {}
        self._load_existing()

    def append(self, receipt: WalletRelationshipIntelligenceAuditReceipt) -> str:
        if not isinstance(receipt, WalletRelationshipIntelligenceAuditReceipt):
            raise TypeError("receipt must be WalletRelationshipIntelligenceAuditReceipt")
        existing = self._receipts.get(receipt.audit_id)
        if existing is not None:
            if existing != receipt:
                raise RuntimeError("wallet relationship intelligence audit identity collision")
    ...
    def get(self, audit_id: str) -> WalletRelationshipIntelligenceAuditReceipt | None:
        return self._receipts.get(audit_id)

    def iter_receipts(self) -> Iterator[WalletRelationshipIntelligenceAuditReceipt]:
    ...
    def _load_path(
        self, path: Path
    ) -> dict[str, WalletRelationshipIntelligenceAuditReceipt]:
        raw = path.read_text(encoding="utf-8")
        document = json.loads(raw)
        if not isinstance(document, dict) or document.get("schema_version") != _SCHEMA_VERSION:
            raise ValueError("invalid wallet relationship intelligence audit store schema")
        records = document.get("receipts")
        if not isinstance(records, list) or document.get("content_hash") != _hash(records):
            raise ValueError("wallet relationship intelligence audit store content hash mismatch")
        if canonical_json(document) != raw:
            raise ValueError("wallet relationship intelligence audit store is not canonical JSON")
        loaded: dict[str, WalletRelationshipIntelligenceAuditReceipt] = {}
        for record in records:
            ...
            if receipt.audit_id in loaded:
                raise ValueError("duplicate wallet relationship intelligence audit receipt")
            loaded[receipt.audit_id] = receipt
        return loaded

    def _persist(
        self, receipts: tuple[WalletRelationshipIntelligenceAuditReceipt, ...]
    ) -> None:
...
__all__ = ["JsonWalletRelationshipIntelligenceAuditStore"]
