...
class JsonDashboardSessionFinalRecoveryChainReplayStore:
    ...
    def __init__(self, file_path: str | os.PathLike[str]) -> None:
        ...
        self._receipts: dict[
            str, DashboardSessionFinalRecoveryChainReplayReceipt
        ] = {}
        self._load()

    def append(
        self,
        receipt: DashboardSessionFinalRecoveryChainReplayReceipt,
    ) -> str:
        if not isinstance(
            receipt, DashboardSessionFinalRecoveryChainReplayReceipt
        ):
            raise TypeError(
                "receipt must be a DashboardSessionFinalRecoveryChainReplayReceipt"
            )
        existing = self._receipts.get(receipt.verification_id)
        if existing is not None:
            if existing != receipt:
                raise RuntimeError(
                    "session final recovery chain replay identity collision"
                )
    ...
    def get(
        self,
        verification_id: str,
    ) -> DashboardSessionFinalRecoveryChainReplayReceipt | None:
        return self._receipts.get(verification_id)

    def iter_receipts(
        self,
    ) -> Iterator[DashboardSessionFinalRecoveryChainReplayReceipt]:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "session final recovery chain replay store is not valid JSON"
            ) from exc
        if not isinstance(document, dict) or set(document) != {
            ...
            raise ValueError(
                "session final recovery chain replay store keys do not match schema"
            )
        if document["schema_version"] != _SCHEMA_VERSION:
            raise ValueError(
                "unsupported session final recovery chain replay store schema_version"
            )
        ...
        if not isinstance(records, list) or _digest(records) != document[
            ...
            raise ValueError(
                "session final recovery chain replay store content hash mismatch"
            )
        if canonical_json(document) != raw:
            raise ValueError(
                "session final recovery chain replay store is not canonical JSON"
            )
        loaded: dict[str, DashboardSessionFinalRecoveryChainReplayReceipt] = {}
        for record in records:
            try:
                ...
                raise ValueError(
                    "invalid session final recovery chain replay receipt"
                ) from exc
            if receipt.verification_id in loaded:
                raise ValueError(
                    "duplicate session final recovery chain replay verification ID"
                )
    ...
    def _persist(
        self,
        receipts: tuple[DashboardSessionFinalRecoveryChainReplayReceipt, ...],
    ) -> None:
