...
_SCHEMA_VERSION = (
    "dashboard_session_final_recovery_gate_audit_session_integration_mapping_replay_store.v1"
)


class JsonDashboardSessionFinalRecoveryGateAuditSessionIntegrationMappingReplayStore:
    """Atomic persistence for final recovery gate integration mapping replay receipts."""
    ...
    def append(
        self,
        receipt: DashboardSessionFinalRecoveryGateAuditSessionIntegrationMappingReplayReceipt,
    ) -> str:
        if not isinstance(
            receipt,
            DashboardSessionFinalRecoveryGateAuditSessionIntegrationMappingReplayReceipt,
        ...
        if existing is not None:
            if existing != receipt:
                raise RuntimeError(
                    "session final recovery gate integration mapping replay receipt "
                    "identity collision"
                )
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "session final recovery gate integration mapping replay store is not valid JSON"
            ) from exc
        ...
        if not isinstance(document, dict) or set(document) != {
            ...
            raise ValueError(
                "session final recovery gate integration mapping replay store keys do not match schema"
            )
        if document["schema_version"] != _SCHEMA_VERSION:
            raise ValueError(
                "unsupported session final recovery gate integration mapping replay store "
                "schema_version"
            )
        records = document["receipts"]
        if not isinstance(records, list) or _digest(records) != document["content_hash"]:
            raise ValueError(
                "session final recovery gate integration mapping replay store content hash mismatch"
            )
        if canonical_json(document) != raw:
            raise ValueError(
                "session final recovery gate integration mapping replay store is not canonical "
                "JSON"
            )
        ...
        for record in records:
            try:
                ...
                raise ValueError(
                    "invalid session final recovery gate integration mapping replay receipt"
                ) from exc
            if receipt.verification_id in loaded:
                raise ValueError(
                    "duplicate session final recovery gate integration mapping replay "
                    "verification ID"
                )
...
__all__ = [
    "JsonDashboardSessionFinalRecoveryGateAuditSessionIntegrationMappingReplayStore",
]
