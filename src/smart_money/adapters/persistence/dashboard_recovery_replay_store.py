...
class JsonDashboardRecoveryReplayStore:
    ...
    def __init__(self, file_path: str | os.PathLike[str]) -> None:
        ...
        self._receipts: dict[str, DashboardRecoveryReplayReceipt] = {}
        self._load()

    def append(self, receipt: DashboardRecoveryReplayReceipt) -> str:
        if not isinstance(receipt, DashboardRecoveryReplayReceipt):
            raise TypeError("receipt must be a DashboardRecoveryReplayReceipt")
        existing = self._receipts.get(receipt.verification_id)
        if existing is not None:
            if existing != receipt:
                raise RuntimeError("recovery replay identity collision")
    ...
    def get(
        self,
        verification_id: str,
    ) -> DashboardRecoveryReplayReceipt | None:
        return self._receipts.get(verification_id)

    def iter_receipts(self) -> Iterator[DashboardRecoveryReplayReceipt]:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ValueError("recovery replay store is not valid JSON") from exc
        if not isinstance(document, dict) or set(document) != {
            "content_hash",
            "receipts",
            "schema_version",
        }:
            raise ValueError("recovery replay store keys do not match schema")
        if document["schema_version"] != _SCHEMA_VERSION:
            raise ValueError("unsupported recovery replay store schema_version")
        records = document["receipts"]
        if not isinstance(records, list) or _digest(records) != document[
            "content_hash"
        ]:
            raise ValueError("recovery replay store content hash mismatch")
        if canonical_json(document) != source.read_text(encoding="utf-8"):
            raise ValueError("recovery replay store is not canonical JSON")
        loaded: dict[str, DashboardRecoveryReplayReceipt] = {}
        for record in records:
            try:
                receipt = DashboardRecoveryReplayReceipt(
                ...
                raise ValueError("invalid recovery replay receipt") from exc
            if receipt.verification_id in loaded:
                raise ValueError("duplicate recovery replay verification ID")
    ...
    def _persist(
        self,
        receipts: tuple[DashboardRecoveryReplayReceipt, ...],
    ) -> None:
