...
def test_wallet_relationship_query_final_store_replay_verification(tmp_path) -> None:
    identity = {
        "accepted": True,
        "audit_id": "audit-1",
        "gate_id": "gate-1",
    ...
    store = JsonWalletRelationshipIntelligenceQueryFinalStore(
        tmp_path / "query-final.json"
    )
    store.append(receipt)
    verification = verify_wallet_relationship_intelligence_query_final_store(
        store, receipt
    )
