...
from smart_money.application.funding_cluster_store_replay import replay_verify_funding_cluster_store
...
def test_funding_cluster_store_replays_from_relationships(tmp_path) -> None:
    ...
    store = JsonFundingClusterReadModelStore(tmp_path / "clusters.json")
    store.save(model)
    receipt = replay_verify_funding_cluster_store(store, tuple(reversed(relationships)))
