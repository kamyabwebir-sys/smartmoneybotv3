...
from smart_money.application.token_safety_candidate_summary_ledger import (
...
def test_candidate_summary_projection_is_idempotent_and_preserves_provenance():
    candidate_identity = {
    ...
    candidate = SolanaWalletTokenCandidate(
        ...
        deterministic_id("solana_wallet_token_candidate", candidate_identity),
    )
    ...
    safety_binding = bind_token_safety_to_candidate(candidate, observation)
    binding = bind_candidate_safety_summary(candidate, safety_binding, summary)
    ledger = EvidenceGroundingLedger()
    first = ingest_token_safety_candidate_summary_binding(binding, ledger)
    second = ingest_token_safety_candidate_summary_binding(binding, ledger)
    ...
    assert payload.metadata["provenance"]["summary_id"] == summary.summary_id
