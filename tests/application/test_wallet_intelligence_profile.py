...
def test_wallet_intelligence_profile_aggregates_observations_deterministically() -> None:
    identity = {
    ...
    assert profiles[0].wallet == "W"
    assert profiles[0].activity_count == 3
    assert profiles[0].buy_count == 2
    assert profiles[0].observation_ids == (observation.observation_id,)
