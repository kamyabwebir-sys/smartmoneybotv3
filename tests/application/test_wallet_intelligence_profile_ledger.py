...
def test_wallet_intelligence_profile_is_projected_as_replayable_evidence() -> None:
    identity = {
    ...
    receipt = ingest_wallet_intelligence_profile(profile, ledger)
    payload = ledger.get(receipt.evidence_id)
    assert payload is not None
    assert payload.evidence_type == "wallet_intelligence_profile"
    assert payload.data["wallet_intelligence_profile"]["profile_id"] == profile.profile_id
    assert payload.metadata["provenance"]["source_schema_version"] == profile.schema_version
    assert ingest_wallet_intelligence_profile(profile, ledger).already_present is True
