...
from smart_money.adapters.persistence.dashboard_session_final_audit_chain_replay_store import (
    JsonDashboardSessionFinalAuditChainReplayStore,
)
...
def _receipt():
    ...
    return DashboardSessionFinalAuditChainReplayVerifier().verify(chain, chain)


def test_append_is_idempotent_and_reloads(tmp_path) -> None:
    path = tmp_path / "replay.json"
    receipt = _receipt()
    store = JsonDashboardSessionFinalAuditChainReplayStore(path)

    assert store.append(receipt) == receipt.verification_id
    assert store.append(receipt) == receipt.verification_id
    restored = JsonDashboardSessionFinalAuditChainReplayStore(path)
...
def test_hash_corruption_is_rejected(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="content hash mismatch"):
        JsonDashboardSessionFinalAuditChainReplayStore(path)
