from __future__ import annotations

import json

import pytest

from smart_money.adapters.persistence import (
    dashboard_session_final_recovery_gate_audit_chain_store as chain_store,
)
from smart_money.application.dashboard_session_final_recovery_gate_audit_chain import (
    DashboardSessionFinalRecoveryGateAuditChain,
)


def _chain() -> DashboardSessionFinalRecoveryGateAuditChain:
    return DashboardSessionFinalRecoveryGateAuditChain(
        entries=(),
        chain_hash="0" * 64,
    )


def test_append_is_idempotent_and_reloads(tmp_path) -> None:
    path = tmp_path / "chains.json"
    chain = _chain()
    store = chain_store.JsonDashboardSessionFinalRecoveryGateAuditChainStore(path)

    assert store.append(chain) == chain.chain_id
    assert store.append(chain) == chain.chain_id
    restored = chain_store.JsonDashboardSessionFinalRecoveryGateAuditChainStore(
        path
    )

    assert restored.chain_count == 1
    assert restored.get(chain.chain_id) == chain
    assert list(restored.iter_chains()) == [chain]


def test_hash_corruption_is_rejected(tmp_path) -> None:
    path = tmp_path / "chains.json"
    store = chain_store.JsonDashboardSessionFinalRecoveryGateAuditChainStore(path)
    store.append(_chain())
    document = json.loads(path.read_text(encoding="utf-8"))
    document["content_hash"] = "f" * 64
    path.write_text(json.dumps(document), encoding="utf-8")

    with pytest.raises(ValueError, match="content hash mismatch"):
        chain_store.JsonDashboardSessionFinalRecoveryGateAuditChainStore(path)
