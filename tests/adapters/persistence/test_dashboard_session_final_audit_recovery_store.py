...
from smart_money.adapters.persistence.dashboard_session_final_audit_recovery_store import (
    JsonDashboardSessionFinalAuditRecoveryStore,
)
...
def _receipt():
    ...
    return DashboardSessionFinalAuditRecoveryGate().evaluate(
        chain, replay, replay
    )
...
def test_append_is_idempotent_and_reloads(tmp_path) -> None:
    ...
    receipt = _receipt()
    store = JsonDashboardSessionFinalAuditRecoveryStore(path)

    assert store.append(receipt) == receipt.receipt_id
    assert store.append(receipt) == receipt.receipt_id
    restored = JsonDashboardSessionFinalAuditRecoveryStore(path)
...
def test_hash_corruption_is_rejected(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="content hash mismatch"):
        JsonDashboardSessionFinalAuditRecoveryStore(path)
